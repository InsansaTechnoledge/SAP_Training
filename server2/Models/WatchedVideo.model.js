...
WatchedVideoSchema.pre('save', function(next) {
    ...
    const recencyFactor = Math.min(1, (Date.now() - this.firstWatched) / (1000 * 60 * 60 * 24 * 30));
...
WatchedVideoSchema.methods = {
    ...
    updateProgress: async function(currentTimeInSeconds, duration) {
        ...
        const newProgress = Math.min(100, Math.round((currentTimeInSeconds / duration) * 100));
    ...
    endSession: async function(currentTimeInSeconds) {
        ...
        if (lastSessionIndex >= 0 && !this.watchSessions[lastSessionIndex].endTime) {
            ...
            this.watchSessions[lastSessionIndex].progressMade = this.progress - session.progressMade;
...
WatchedVideoSchema.statics = {
    ...
    async scheduleResumeReminders() {
        ...
        for (const video of candidates) {
            ...
            console.log(`Sent reminder to ${video.userId.email} to resume video ${video.videoId}`);
            count++;
        }
    ...
    async generateUserVideoStats(userId) {
        return this.aggregate([
            ...
            { $group: {
                _id: null,
                totalVideos: { $sum: 1 },
                completedVideos: { $sum: { $cond: [{ $eq: ["$completed", true] }, 1, 0] } },
    ...
    async getCourseCompletion(userId, courseId) {
        ...
        return {
            completion: (result[0].completedVideos / result[0].totalVideos) * 100,
    ...
    async estimateRemainingWatchTime(userId, courseId) {
        ...
        for (const video of unwatched) {
            ...
            const videoRemainingSeconds = (remainingPercentage / 100) * videoDurationSeconds;
