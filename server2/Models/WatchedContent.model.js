...
const WatchedContentSchema = new mongoose.Schema({
    ...
    status: {
        type: String,
        enum: ['not_started', 'in_progress', 'completed', 'abandoned', 'expired', 'needs_review'],
        default: 'not_started'
    },
...
WatchedContentSchema.index({ userId: 1, courseId: 1, moduleId: 1, contentId: 1 }, { unique: true });
...
WatchedContentSchema.pre('save', function(next) {
    ...
    if (this.isModified('watchProgress')) {
        if (this.watchProgress > 0 && this.watchProgress < 100 && this.status !== 'completed') {
...
WatchedContentSchema.statics = {
    ...
    async scheduleReminders() {
        ...
        for (const content of needReminders) {
            ...
            console.log(`Reminder sent to user ${content.userId} for course ${content.courseId}`);
            remindersSent++;
        }
...
export const setupWatchedContentAutomation = () => {
    ...
    cron.schedule('0 0 * * *', async () => {
        try {
            ...
            console.log(`Automation complete: ${abandoned} marked abandoned, ${expired} expired, ${flagged} flagged for review`);
        } catch (error) {
